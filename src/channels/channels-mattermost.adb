...
package body Channels.Mattermost
  with SPARK_Mode => Off
is
   use Config.Schema;
   ...
   is
      Chan_Cfg   : constant Config.Schema.Channel_Config :=
        Find_Channel (Cfg, Config.Schema.Mattermost);
      Bridge_URL : constant String :=
        To_String (Chan_Cfg.Bridge_URL);
      Poll_State : Channels.Bridge_Polling.Backoff_State;
   begin
      if not Chan_Cfg.Enabled
        or else Bridge_URL'Length = 0
      then
         Logging.Info ("Mattermost: not configured, skipping.");
         return;
      end if;
      ...
      Logging.Info ("Mattermost: polling " & Bridge_URL & " ...");
                     ...
                     if PR.Valid then
                        ...
                        begin
                           for I in 1 .. Array_Length (Root_Arr) loop
                              ...
                              begin
                                 if Channels.Message_Dedup.Was_Seen
                                   (Seen, Msg_ID)
                                 then
                                    goto Next_Item;
                                 end if;
                                 Channels.Message_Dedup.Mark_Seen
                                   (Seen, Msg_ID);
                                 ...
                                 if Text'Length > 0 then
                                    declare
                                       ...
                                       Matches : constant Boolean :=
                                         Allowlist = "*"
                                         or else Index
                                           (Allowlist, From_User) > 0;
                                    begin
                                       if not
                                         Channels.Security.Allowlist_Allows
                                           (Channel =>
                                              Channels.Security
                                                .Chat_Channel,
                                            Allowlist_Size =>
                                    ...
                                    begin
                                       ...
                                       if Memory.SQLite.Is_Open (Mem)
                                       then
                                          Memory.SQLite.Load_History
                                            (Mem,
                                             "mattermost:" & From_User,
                                             Cfg.Memory.Max_History,
                                             Conv);
                                       end if;
